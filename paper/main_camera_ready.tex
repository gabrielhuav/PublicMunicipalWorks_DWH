...
\begingroup\revcol
The static artefact is permanently online, so its delivery can be audited by anyone against a public URL. Table~\ref{tab:lighthouse} reports Google Lighthouse over its two entry points, the landing page and the vendored viewer. These are measurements in the sense of Section~\ref{sec:alcance}, not targets.
...
Both pages score 100 on accessibility, leave the main thread idle and shift no layout, so only delivery weight separates them: the landing page waits on render-blocking web fonts and a CDN animation library, the map on a 533~kB bundle. Neither is a property of the warehouse: both are traits of an unoptimised front end, remediable without touching the architecture.

Conditions: Lighthouse 12.8.2, headless Chrome 151, emulated mobile with simulated throttling, 11 August 2026, tag \texttt{v1.4.0-icokg2026}; the artefact is static, so a re-run audits the same page. Lighthouse measures delivery, not query latency.
\endgroup
...
the authors, and writing the auxiliary scripts distributed with the artefact\rev{, and generating the artefact's illustrative work images, each labelled as such in the file}. The
