...
\caption{Comparison with representative systems. SCD~2 $=$ Slowly Changing Dimension Type~2; ``Ev.'' $=$ managed bidirectional link between binary field evidence and the analytical store; ``Sem.'' $=$ semantic layer.}
...
The warehouse adopts a dimensional star model~\cite{Kimball2013} with two fact tables and ten dimensions (Table~\ref{tab:dimensiones}), shown schematically in Figure~\ref{fig:esquema_warehouse}.
...
\caption{Lighthouse audit of the published artefact, median of three runs per page. Reproducible over the public URL.}
...
\caption{Detection performance against an independently injected ground truth (synthetic dataset, 1,421 records, seed 42). Reproducible with the published script.}
