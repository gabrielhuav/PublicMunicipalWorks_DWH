...
Over a seeded test scenario, the prototype organizes 1,247 simulated public works with an accumulated budget of \$127.4~million pesos, 8,934 simulated audit events, and 3,421 georeferenced photographic pieces of evidence. \rev{All figures reported in this article were obtained over synthetic data generated with a fixed seed; they characterise the behaviour of the system, not the municipality of Temascaltepec, and no claim about real public-works execution in that municipality is made or implied.}
...
\begingroup\revcol
The warehouse DDL, SCD~2 triggers, seeded generators, load-testing scripts and the evaluation script are archived at {\small\url{https://github.com/gabrielhuav/PublicMunicipalWorks_DWH}}. A permanently deployable static demonstration is published through GitHub Pages at {\small\url{https://gabrielhuav.github.io/PublicMunicipalWorks_DWH/}}. It preserves the HTML/CSS/JavaScript visual interface of the original public-works prototype, including its role-access, budgeting, supervision, records and citizen-participation views, while replacing remote authentication and write operations with synthetic in-browser demonstration behaviour. It therefore makes no request to Flask, Python, Render, Supabase or a live API. Because that deployment has no server, access to it is open by construction: the demonstration credentials are printed on the landing page and the sign-in form also accepts any value, including an empty one. Every screen is served by a synthetic dataset held in the visitor's own browser tab, so no input can affect what another visitor sees, and closing the tab restores the seeded state. The Flask implementation in \texttt{backend/} remains the operational reference, where the same \texttt{DEMO-} accounts are constrained to read-only access. Table~\ref{tab:eval-deteccion-anomalias} is reproduced by running \texttt{generar\_dataset\_obras.py} and \texttt{eval\_deteccion\_v2.py} from \texttt{scripts/evaluacion/}, both with seed 42.
\endgroup
...
Armbrust, M., Das, T., Sun, L., Yavuz, B., Zhu, S., Murthy, M., Torres, J., van Hovell, H., Ionescu, A., \L{}uszczak, A., \'Switakowski, M., Szafra\'nski, M., Li, X., Ueshin, T., Mokhtar, M., Boncz, P., Ghodsi, A., Paranjpye, S., Senster, P., Xin, R., Zaharia, M.: Delta Lake: high-performance ACID table storage over cloud object stores. Proc. VLDB Endow. \textbf{13}(12), 3411--3424 (2020). \url{https://doi.org/10.14778/3415478.3415560}
