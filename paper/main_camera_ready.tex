...
\centering
\revcol
\caption{\rev{Lighthouse audit of the published artefact, median of three runs per page. Reproducible over the public URL.}}
...
Both pages score 100 on accessibility, leave the main thread idle and shift no layout, so only delivery weight separates them: the landing page waits on render-blocking web fonts and a CDN animation library, the map on a 533~kB bundle. Neither is a property of the warehouse: both are delivery traits of an unoptimised front end, remediable without touching the architecture. Lighthouse audits \emph{page delivery}, not query latency; the latter is in Section~\ref{sec:objetivos} and needs the deployed backend.

Conditions: Lighthouse 12.8.2, headless Chrome 151, emulated mobile device with simulated throttling, 11 August 2026, tag \texttt{v1.3.1-icokg2026}. The artefact is static, so re-running the audit yields the same page that produced these figures.
\endgroup
